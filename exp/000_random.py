...
def k_fold(num_fold, all_df, seed):
    skf = KFold(num_fold, shuffle=True, random_state=seed)
...
def main(config: DictConfig) -> None:
    ...
    for fold, (train_idx, val_idx, test_idx) in enumerate(zip(*k_fold(config.train.num_folds, all_df, config.seed))):
        model = LightGCN(
