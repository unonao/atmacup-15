...
def objective(trial, X, y, train_user_ids, config):
    seed_everything()
    # LightGBM parameters
    lgb_params = {
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(X, y, train_user_ids)):
        if fold > 0:
            break
...
def main(config: DictConfig) -> None:
    ...
    study.optimize(lambda trial: objective(trial, X_train_all, y_train_all, train_user_ids, config), n_trials=n_trials)
