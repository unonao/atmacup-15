...
def main(config: DictConfig) -> None:
    ...
    nn_feas_dir = Path(f"../nn_feats/{dict_to_hash(OmegaConf.to_container(config.nn.feats))}")
    os.makedirs(nn_feas_dir, exist_ok=True)
    train_path = nn_feas_dir / "train.feather"
    test_path = nn_feas_dir / "test.feather"
    if os.path.exists(train_path) and os.path.exists(test_path):
        X_train_all = pd.read_feather(train_path)
        X_test = pd.read_feather(test_path)
    else:
        # 指定した特徴量からデータをロード
        X_train_all, X_test = load_datasets(config.nn.feats)
        X_train_all, X_test = prepare_data(X_train_all, X_test)
        X_train_all.reset_index(drop=True).to_feather(train_path)
        X_test.reset_index(drop=True).to_feather(test_path)
    ...
    for fold, (train_index, valid_index) in enumerate(kf.split(X_train_all, y_train_all, train_user_ids)):
        ...
        model = MyNet(
            ...
            n_layers=config.nn.num_layers,
            hidden_dim=config.nn.hidden_dim,
            dropout_rate=config.nn.dropout_rate,
        ).to(device)
        ...
        lr_sched = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer, T_0=config.nn.t0, T_mult=config.nn.t_mult, eta_min=config.nn.lr * config.nn.eta_min_rate
        )
