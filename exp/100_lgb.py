...
def main(config: DictConfig) -> None:
    ...
    if config.debug:
        X_train_all = X_train_all.head(100)
        y_train_all = y_train_all.head(100)
    ...
    print({"rmse/all_val": rmse_all_valid})
    ...
    sub = load_sample_sub()
    sub[config.lgb.target_name] = np.mean(mean_y_preds, axis=0)
